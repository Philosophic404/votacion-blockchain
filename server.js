...
if (!PRIVATE_KEY) {
    console.error("❌ PRIVATE_KEY no definida en .env");
    process.exit(1);
}
...
app.post("/api/admin/login", (req, res) => {
    ...
    if (!password) return res.json({ exito: false, mensaje: "Ingresa la contraseña." });
    if (password !== ADMIN_PASSWORD) return res.json({ exito: false, mensaje: "Contraseña incorrecta." });
...
app.post("/api/verificar-cedula", (req, res) => {
    ...
    if (!cedula || cedula.length !== 10 || !/^\d+$/.test(cedula))
        return res.json({ valido: false, mensaje: "Cédula inválida." });
...
app.get("/api/resultados", async (req, res) => {
    try {
        ...
        let i = 0;
        while (true) {
            try { opciones.push(await contratoCompleto.candidatos(i)); i++; }
            catch (e) { break; }
        }
...
app.get("/api/info-proceso", async (req, res) => {
    try {
        ...
        if (fs.existsSync("./proceso-config.json")) {
            descripcion = JSON.parse(fs.readFileSync("./proceso-config.json", "utf8")).descripcion || "";
        }
        res.json({
...
app.post("/api/subir-documento", uploadMultiple.array("pdfs", 10), (req, res) => {
    if (!req.files || req.files.length === 0)
        return res.json({ exito: false, mensaje: "No se recibieron archivos." });
    const guardados = [];
    req.files.forEach(file => {
        const destino = path.join("./documentos-proceso", file.originalname);
...
app.get("/api/documentos-proceso", (req, res) => {
    const archivos = fs.readdirSync("./documentos-proceso").filter(f => f.endsWith(".pdf"));
    res.json({ exito: true, documentos: archivos });
});

app.get("/api/documento-proceso/:nombre", (req, res) => {
    const ruta = path.join("./documentos-proceso", req.params.nombre);
...
const uploadPadron = multer({ dest: "./padrones/" });
app.post("/api/subir-padron", uploadPadron.single("padron"), (req, res) => {
    const tipo = req.body.tipo || req.query.tipo;
    const nombres = {
        "civil": "padrones/registro_civil.xlsx",
        "cne": "padrones/cne.xlsx",
        "partido": "padrones/partido.xlsx"
    };
    if (!nombres[tipo]) return res.json({ exito: false, mensaje: "Tipo inválido. Usa: civil, cne o partido." });
    if (!req.file) return res.json({ exito: false, mensaje: "No se recibió ningún archivo." });
    fs.renameSync(req.file.path, nombres[tipo]);
    res.json({ exito: true, mensaje: `Padrón "${tipo}" actualizado correctamente.` });
});
...
app.post("/api/crear-proceso", uploadProceso.array("pdfs", 10), async (req, res) => {
    ...
    try {
        if (req.files && req.files.length > 0) {
            req.files.forEach(file => {
                fs.renameSync(file.path, path.join("./documentos-proceso", file.originalname));
            });
        }
        ...
        // En producción (Sepolia), el deploy se hace manualmente. En local, automático.
        const network = process.env.HARDHAT_NETWORK || "localhost";
        const output = execSync(`npx hardhat run scripts/deploy.js --network ${network}`, { encoding: "utf8" });
...
app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`Red: ${process.env.RPC_URL?.includes("sepolia") ? "Sepolia ✓" : "localhost"}`);
    console.log(`Contrato: ${CONTRATO_ACTIVO}\n`);
});
